...
for( i_class in c(0)){ 
	...
	for( o_subset in c("Clinical","Clinical_COG","COG")){ #c("All","Clinical","Serum","Genetic_Epigenetic")
		...
		for( p_algo in c("rf")){ # svmLinear/glmnet problems ; c("cforest","svmLinear","glmnet", "rf")
			#browser()
			# Show progress
...
# make Clinical
g_Clinical = pROC::roc(get(paste0("ROC_",temp_Outcome,"_",o_subset1,"_","rf"))$Obs_C, get(paste0("ROC_",temp_Outcome,"_",o_subset1,"_","rf"))$Pred_P[,s_ROC_Level], plot=FALSE, legacy.axes=FALSE, percent=TRUE, lwd=2, print.auc=TRUE,direction=s_ROC_direction,color="white")


# make Clinical_COG
g_Clinical_COG = pROC::roc(get(paste0("ROC_",temp_Outcome,"_",o_subset2,"_","rf"))$Obs_C, get(paste0("ROC_",temp_Outcome,"_",o_subset2,"_","rf"))$Pred_P[,s_ROC_Level], plot=FALSE, legacy.axes=FALSE, percent=TRUE, lwd=2, print.auc=TRUE,direction=s_ROC_direction)


# make COG
g_COG = pROC::roc(get(paste0("ROC_",temp_Outcome,"_",o_subset3,"_","rf"))$Obs_C, get(paste0("ROC_",temp_Outcome,"_",o_subset3,"_","rf"))$Pred_P[,s_ROC_Level], plot=FALSE, legacy.axes=FALSE, percent=TRUE, lwd=2, print.auc=TRUE,direction=s_ROC_direction,colour="red")
...
# Prepare MCCs
MCC_cforest = as.numeric(get(paste0("ROC_",temp_Outcome,"_",o_subset1,"_","rf"))$Metrics["MCC"])
if(MCC_cforest==-1){MCC_cforest=NA}

MCC_svmLinear = as.numeric(get(paste0("ROC_",temp_Outcome,"_",o_subset2,"_","rf"))$Metrics["MCC"])
if(MCC_svmLinear==-1){MCC_svmLinear=NA}

MCC_glmnet = as.numeric(get(paste0("ROC_",temp_Outcome,"_",o_subset3,"_","rf"))$Metrics["MCC"])
...
# full table
g_total = pROC::ggroc(list(Clinical = g_Clinical, Clinical_COG = g_Clinical_COG, COG = g_COG), size = s_size,aes = c("linetype","colour","size"),legacy.axes = FALSE)+
scale_color_manual(name = "Feature set: ",values=c("Clinical" = s_colorset[1],"Clinical_COG" = s_colorset[2], "COG" = s_colorset[3]))+
scale_linetype_manual(name = "Feature set: ", values=c("Clinical" = 1,"Clinical_COG" = 2, "COG" = 3))+
...
# svmLinear
annotate("text", x = s_AUC_x, y = s_Text_top-2*s_Text_step, label = paste0(sprintf("%.2f",round(g_Clinical_COG$auc[1]/100,3))),col=s_colorset[2], fontface =ifelse(model_best==2,s_BoldThickFont,"plain"),hjust = 1)+
#glmnet
annotate("text", x = s_AUC_x, y = s_Text_top-3*s_Text_step, label = paste0(sprintf("%.2f",round(g_COG$auc[1]/100,3))),col=s_colorset[3], fontface = ifelse(model_best==3,s_BoldThickFont,"plain"),hjust = 1)+
# 			MCCs 
#cforest
...
# svmLinear
annotate("text", x = s_MCC_x, y = s_Text_top-2*s_Text_step, label = paste0(sprintf("%.2f",round(MCC_svmLinear,digits = 2))),col=s_colorset[2], fontface =ifelse(model_best==2,s_BoldThickFont,"plain"),hjust = 1)+
#glmnet
annotate("text", x = s_MCC_x, y = s_Text_top-3*s_Text_step, label = paste0(sprintf("%.2f",round(MCC_glmnet,digits = 2))),col=s_colorset[3], fontface = ifelse(model_best==3,s_BoldThickFont,"plain"),hjust = 1)+
# Theme ; legend
...
pdf(paste0(ROC_Rootdir,"/MultiROC_Main_",format(Sys.time(), "%M%S_%d%m%y"),".pdf"), width = 16) # Open a new pdf file
	...
	grid.text("Cognitive feature sensivity", 	x = 0.5, y = row_y,rot=0,gp = gpar(fontface = "bold"),just="center")
