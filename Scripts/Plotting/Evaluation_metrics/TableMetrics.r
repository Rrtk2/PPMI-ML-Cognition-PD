...
CM_table = data.frame(matrix(nrow=2*4*4,ncol=20+7+2))
...
for( i_class in c(0,1)){ 
	...
	for( o_subset in c("All","Clinical","Serum","Genetic_Epigenetic")){ #c("All","Clinical","Serum","Genetic_Epigenetic")
		...
		for( p_algo in c("cforest","svmLinear","glmnet", "rf")){ # svmLinear/glmnet problems ; c("cforest","svmLinear","glmnet", "rf")
			...
			# assign; but can fail
			TN=minitable["Class0","Class0"] 
			TP=minitable["Class1","Class1"]
...
#							save
#-----------------------------------------------------------------------------------------------------#
write.table(CM_table,file = paste0(CM_Rootdir,"/Table_metrics_full_",format(Sys.time(), "%M%S_%d%m%y"),".txt"),sep = "\t",quote = FALSE,row.names=FALSE)
write.table(CM_table_MAIN,file = paste0(CM_Rootdir,"/Table_metrics_main_",format(Sys.time(), "%M%S_%d%m%y"),".txt"),sep = "\t",quote = FALSE,row.names=FALSE)